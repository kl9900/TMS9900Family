...
*      MOV  R1,R2             Copy for later use.                         *TOMY*
       SLA  R1,2              Check for UDF entry
       JOC  BERMUV            If UDF - then error
*      MOV  R4,R4             Check for string.                           *TOMY*
*      JLT  SMB02             Skip if it is string.                       *TOMY*
*      CLR  @FAC2             Clear for numeric case.                     *TOMY*
*
...
*
*SMB04 MOVB R4,R4             R4 has header byte information.             *TOMY*
*      JLT  SMBO50            Take care of string.                        *TOMY*
*SMB05 SLA  R4,5              Get only the dimension number.              *TOMY*
*      SRL  R4,13                                                         *TOMY*
*      JNE  SMBO20             go to array case.                          *TOMY*
*
* Numeric ERAM cases are special.
* If it is shared get the actual v.s. address from ERAM.
* Otherwise get it from VDP RAM.
*
*      MOVB @RAMTOP,R4        Check for ERAM.                             *TOMY*
*      JEQ  SMBO10            Yes ERAM case.                              *TOMY*
*      SLA  R2,3              R2 has a header byte.                       *TOMY*
*      JNC  SMB06             Shared bit is not ON.                       *TOMY*
*      BL   @GETG             Get v.s. pointer from ERAM                  *TOMY*
*      DATA FAC4                                                          *TOMY*
*      JMP  SMB08                                                         *TOMY*
*SMB06 BL   @GET              Not shared.                                 *TOMY*
*      DATA FAC4              Get v.s. address from VDP RAM.              *TOMY*
*                                                                         *TOMY*
*SMB08 MOV  R1,@FAC4          Store it in FAC4 area.                      *TOMY*
SMB04  BL   @GETV                                                         *TOMY*
       SOCB R10,R1                                                        *TOMY*
       JLT  >9212                                                         *TOMY*
       CLR  @FAC2                                                         *TOMY*
...
BIT2   EQU  $+1               Constant >20 (Opcode is >D120)              *TOMY*
...
*
SMBO20  MOV R1,@FAC2          Now have a dimension counter                *TOMY*
...
*BIT2  EQU  $+1               Constant >20 (Opcode is >D120)              *TOMY*
*      MOVB @BASE,R4          Fetch option base to check low              *TOMY*
       SB   R4,R4                                                         *TOMY*
...
*      MOVB @RAMTOP,R3        Does ERAM exist?                            *TOMY*
*      JEQ  SMBO71            No                                          *TOMY*
*      BL   @GET              Yes, get the content of value               *TOMY*
*      DATA FAC4               pointer                                    *TOMY*
*                                                                         *TOMY*
*      MOV  R1,@FAC4          Put it in FAC4                              *TOMY*
...
*      DATA ARG               Pointer is in ARG                           *TOMY*
*                                                                         *TOMY*
*      MOV  R1,@ARG4          Correct ARG+4,5 too                         *TOMY*
*-----------------------------------------------------------*
...
*      C    R1,@FAC4          Do not do anything in assign- *             *TOMY*
*                              ing a string to itself case  *
*      JEQ  ASSG80            Detect A$=A$ case, exit       *             *TOMY*
*-----------------------------------------------------------*
       SOCB *R12,R1                                                       *TOMY*
       MOV  R1,@ARG4                                                      *TOMY*
...
*      MOV  @RAMTOP,R3        Does ERAM exist?                            *TOMY*
*      JNE  ASSG77            Yes, write to ERAM                          *TOMY*
*                             No, write to VDP                            *TOMY*
...
*ASSG77 LI   R4,FAC           Source is in FAC                            *TOMY*
*ASSG79 MOVB *R4+,*R5+        Move a byte                                 *TOMY*
*      DEC  R2                Decrement the counter, done?                *TOMY*
*      JGT  ASSG79            No, loop for more                           *TOMY*
*      B    *R10              Yes, return to caller                       *TOMY*
...
PGMCH  MOV  R11,R12           Save return address
       ...
       B    *R12              Return to GPL
*      RT                     And return to the caller                    *TOMY*
PUTV   MOV  *R11+,R4
       MOV  *R4,R4
PUTV1  MOVB @R4LB,*R15
       SWPB R14                                                           *TOMY*
...
*      MOV @RAMTOP,R0         Does ERAM exist?                            *TOMY*
*      JNE MOVFA2             Yes, from ERAM                              *TOMY*
*                             No, from VDP RAM                            *TOMY*
       SWPB R1
