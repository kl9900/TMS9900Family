...
*
CRUNCH MOV  *R13+,R3                                                      *TOMY*
       MOV  R11,R12           Save return link                            *TOMY*
*      MOVB *R13,R3           Read call code                              *TOMY*
...
*      MOVB R8,@PRGFLG        Clear program flag                          *TOMY*
...
*CRU10 CB   *R4,@ENDPRO       Higher then "z"               *             *TOMY*
*      JHE  CRU09             Yes, give CPNIL property      *             *TOMY*
CRU10  EQU  $                                                             *TOMY*
       MOVB @CPTBL(R8),R0     Fetch char's prop table value *
CRU12  B    *R7               Dispatch to appropriate code  *             *TOMY*
*CRU09 MOVB CPNIL,R0          Don't go to CPT, just take    *             *TOMY*
*                              CPNIL prop                   *
*-----------------------------------------------------------*
*CRU12 B    *R7               Dispatch to appropriate code                *TOMY*
...
*ENDPRO BYTE >7B              ASCII code for char after "z" *             *TOMY*
*      EVEN                                                 *             *TOMY*
*-----------------------------------------------------------*
*
...
*      SRL  R0,2              Adjust R0 for LOWUP routine   *             *TOMY*
*      BL   @LOWUP            Translate lowercase to upper  *             *TOMY*
*                              if necessary                 *
*-----------------------------------------------------------*
...
CRU38  MOV  @RAMPTR,R3        Get end pointer
       ...
       A    @>0278,R2                                                     *TOMY*
       MOV  R2,R13                                                        *TOMY*
*      AI   R2,KEYTAB         Add in address of table list                *TOMY*
*      MOVB R2,@GRMWAX(R13)    Load address to GROM                       *TOMY*
*      SWPB R2                                                            *TOMY*
*      MOVB R2,@GRMWAX(R13)                                               *TOMY*
*      MOVB *R13,R2           Read address of correct table               *TOMY*
       MOVB *R13+,R2                                                      *TOMY*
       MOVB *R13,@R2LB        Both bytes
* R2 now contains the address of the correct table
*CRU40 MOVB R2,@GRMWAX(R13)   Load address of table                       *TOMY*
CRU40  MOV  R2,R13                                                        *TOMY*
       MOV  R3,R0             Copy of length for compare
*      MOVB @R2LB,@GRMWAX(R13)                                            *TOMY*
...
CRU42  CB   @XVDPRD,*R13+     Compare the character                       *TOMY*
...
CRU53A MOVB *R0,R1            Pick up offset from table
       ...
       B    @OFFST2(R1)       Goto special case handler                   *TOMY*
...
CRU54  LI   R10,CRU83         Image after, string copy mode               *TOMY*
*      JMP  CRU59             Handle similar to data stmt                 *TOMY*
* Process a DATA statement
*CRU58 LI   R10,CRU83         After-datum skip spaces                     *TOMY*
CRU58  EQU  $                                                             *TOMY*
...
*CRU65 MOV  @RAMPTR,R3        Get the current crunch pointer              *TOMY*
*      DEC  R3                Point at last character put in              *TOMY*
*      BL   @GETV1            Read it                                     *TOMY*
*      CB   R1,@GO$TOK        Was it a GO?                                *TOMY*
*      JEQ  CRU52             Yes, SUB is part of GO SUB                  *TOMY*
* Process a CALL SUB statement
*CRU66 LI   R7,CRU93          Set name copy                               *TOMY*
*      JMP  CRU74             And get next character                      *TOMY*
CRU65  EQU  $                                                             *TOMY*
CRU66  EQU  $                                                             *TOMY*
CRU32L B    @CRU32
...
CRU76  ANDI R0,CPALNM*256     Is this char alpha or digit   *             *TOMY*
       JNE  CRU74             Yes, continue packing         *             *TOMY*
*CRU76 ANDI R0,CPULNM*256     Is this char alpha (both are  *             *TOMY*
*                              upper and lower) or a digit? *
*      JNE  CRU78             Yes, continue packing         *             *TOMY*
*-----------------------------------------------------------*
...
* for 99/4A                   5/12/81                       *
*CRU78 BL   @LOWUP            Translate lower to upper if   *             *TOMY*
*                              necessary                    *
*      JMP  CRU74             Continue packing              *             *TOMY*
*-----------------------------------------------------------*
...
CRU94  ANDI R0,CPALNM*256     Still an alpha-numeric        *             *TOMY*
       JNE  CRU74             Yes, include in name          *             *TOMY*
*CRU94 ANDI R0,CPULNM*256     Still an alpha(U & L)-numeric *             *TOMY*
*      JNE  CRU91             Yes, transfer L to U, then    *             *TOMY*
*                              include in name              *
*-----------------------------------------------------------*
       MOV  R8,R8             At end of line?
...
* for 99/4A                   5/12/81                       *
*CRU91 BL   @LOWUP            Transfer lowercase char to    *             *TOMY*
*                              uppercase char if necessary  *
*      B    @CRU74            Include in name               *             *TOMY*
*-----------------------------------------------------------*
...
* for 99/4A                   5/12/81                       *
       ANDI R0,CPALPH*256    An alphabetic to start name?   *             *TOMY*
*      ANDI R0,CPUL*256       An alphabetic (both U & L) to *             *TOMY*
*                              start name?                  *
*-----------------------------------------------------------*
...
* byte                                                      *
LENGTH   MOV  R11,R3            Save return address         *             *TOMY*
...
GETINT MOV  R11,R3            Save return address
       ...
       LI   R10,10            Get radix in register for speed             *TOMY*
       AI   R0,-'0'           Convert from ASCII to binary
*      C    R0,R2             Is the character a digit?                   *TOMY*
       C    R0,R10                                                        *TOMY*
       JL   GETI02            Yes, there is a number!
       CLR  R0                No, indicate no number
       B    *R3               Done, no number
GETI01 MPY  R10,R0            Multiply previous by radix                  *TOMY*
       MOV  R0,R0             Overflow?
...
*      C    R8,R2             Is this character a digit?                  *TOMY*
       C    R8,R10                                                        *TOMY*
...
*           Offset from label OFFST2 in this assembly of    *             *TOMY*
*           the special case handler                        *
*************************************************************
SPECTB BYTE LIST$,CRU57-OFFST2                                            *TOMY*
       BYTE OLD$,CRU58-OFFST2                                             *TOMY*
       BYTE SAVE$,CRU58-OFFST2                                            *TOMY*
       BYTE MERGE$,CRU58-OFFST2                                           *TOMY*
       AORG >B8FC                                                         *TOMY*
       BYTE SSEP$,CRU53-OFFST2                                            *TOMY*
       BYTE TREM$,CRU74-OFFST2                                            *TOMY*
       BYTE DATA$,CRU58-OFFST2                                            *TOMY*
       BYTE REM$,CRU74-OFFST2                                             *TOMY*
       AORG >B90E                                                         *TOMY*
*      BYTE CALL$,CRU66-OFFST2                                            *TOMY*
*      BYTE SUB$,CRU65-OFFST2                                             *TOMY*
*      BYTE IMAGE$,CRU54-OFFST2                                           *TOMY*
       BYTE >FF
       EVEN
*
...
*       lowercase character
*LOWUP  ANDI R0,CPLOW*256     Is lowercase prop set?                      *TOMY*
*      JEQ  LU01              No, just return                             *TOMY*
*      SB   @CBH20,*R4        Change lower to upper                       *TOMY*
*LU01  RT                                                                 *TOMY*
       AORG >BFFE                                                         *TOMY*
       DATA >A202                                                         *TOMY*
********************************************************************************
