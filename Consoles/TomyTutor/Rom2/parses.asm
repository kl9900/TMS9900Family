...
*
PARSEG MOV  R11,R4                                                        *TOMY*
       BL   @SETREG           Set up registers for Basic                  *TOMY*
       MOV  R13,R11                                                       *TOMY*
       MOV  R4,R13                                                        *TOMY*
*      MOVB @GRMRAX(R13),R11   Get GROM address                           *TOMY*
*      MOVB @GRMRAX(R13),@R11LB                                           *TOMY*
*      DEC  R11                                                           *TOMY*
*
* 9900 ENTRY TO PARSE
*
...
P10    BL   @PGMCHR           Get next character
       ...
       CI   R7,PAD0                                                       *TOMY*
       JL   B9900             If 9900 code                                *TOMY*
P17    EQU  $                 R7 contains offset into nudtab
       ANDI R7,>0FFF          If GPL code, get rid of MSB                 *TOMY*
       A    @NUDTAB,R7        Add in table address
NUDG05 BL   @SAVREG           Restore GPL pointers
       MOV  R7,R13                                                        *TOMY*
*      MOVB R7,@GRMWAX(R13)    Write out new GROM address                 *TOMY*
*      SWPB R7                Bare the LSB                                *TOMY*
*      MOVB R7,@GRMWAX(R13)    Put it out too                             *TOMY*
       B    @RESET            Go back to GPL interpreter
P17L   JMP  P17
*
* CONTINUE ROUTINE FOR PARSE
*
CONTG  MOV  R11,R13                                                       *TOMY*
       BL   @SETREG           GPL entry-set Basic registers               *TOMY*
CONT   MOV  *R9,R6            Get last address from stack
*      JGT  CONT10            9900 code if not negative                   *TOMY*
*      MOVB R6,@GRMWAX(R13)    Write out new GROM address                 *TOMY*
*      SWPB R6                Bare the second byte                        *TOMY*
*      MOVB R6,@GRMWAX(R13)    Put it out too                             *TOMY*
*      MOV  R13,R6            Set up to test precedence                   *TOMY*
...
NUDE10 DECT R9                Back up subroutine stack
       INCT R7                Skip over precedence                        *TOMY*
       JMP  NUDG05            Goto code to return to GPL
NOLEDL B    @NOLED
NUDNDL JMP  NUDND1
* Execute one or more lines of Basic
EXECG  MOV  R11,R13                                                       *TOMY*
*EXECG EQU  $                 GPL entry point for execution               *TOMY*
...
*      LIMI 3                 Let interrupts loose             <          *TOMY*
C0     EQU  $+2               Constant data 0                  <
*      LIMI 0                 Shut down interrupts             <          *TOMY*
*      CLR  @>83D6            Reset VDP timeout                < CRU      *TOMY*
*      LI   R12,>24           Load console KBD address in CRU  < KEY      *TOMY*
*      LDCR @C0,3             Select keyboard section          < SCAN     *TOMY*
*      LI   R12,6             Read address                     < SECTION  *TOMY*
*      STCR R0,8              SCAN the keyboard                < MUST     *TOMY*
*      CZC  @C1000,R0         Shift-key depressed?             < BE       *TOMY*
*      JNE  EXEC16            No, execute the Basic statement  < PATCHED  *TOMY*
*      LI   R12,>24           Test column 3 of keyboard        < TO       *TOMY*
*      LDCR @CB3,3            Select keyboard section          < WORK     *TOMY*
*      LI   R12,6             Read address                     < ON       *TOMY*
*      STCR R0,8              SCAN the keyboard                < A        *TOMY*
*      CZC  @C1000,R0         Shift-C depressed?               < GENEVE   *TOMY*
*      JEQ  BRKP1L            Yes, so take Basic breakpoint    < COMPUTER *TOMY*
       MOVB @>F0C8,R12                                                    *TOMY*
       SLA  R12,2                                                         *TOMY*
       JOC  BRKP1L                                                        *TOMY*
...
EXEC20 MOV  R8,R7             Save 1st token so can get 2nd
       ...
       CI   R7,PAD0                                                       *TOMY*
       JHE  P17L              If top bit set -> GROM code                 *TOMY*
...
CONTIN MOV  R11,R13                                                       *TOMY*
       BL   @SETREG           Set up Basic registers                      *TOMY*
...
*      JLT  NUDE10            If negative - return to GPL                 *TOMY*
       MOV  *R7,R6                                                        *TOMY*
       SLA  R6,8                                                          *TOMY*
       CI   R6,PAD0                                                       *TOMY*
       JH   NUDE10                                                        *TOMY*
...
RTNG   MOV  R11,R13                                                       *TOMY*
       BL   @SETREG           Set up registers again                      *TOMY*
...
*      MOVB @RAMFLG,R4         to call CSN                                *TOMY*
*      JEQ  NUMC49                                                        *TOMY*
*      LI   R3,GETCGR                                                     *TOMY*
...
*      BL   @NUMCHK           Ensure index is a number                    *TOMY*
...
*                              breakpoint checking
       ...
       CB   *R2,R8            2nd byte match?
       JEQ  GOTO36            Yes, line is found!
GOTO33 SWPB R14                                                           *TOMY*
       MOVB *R2,R3            Skip 1st byte of line pointer               *TOMY*
...
*      JEQ  RETU40            Yes, take care of error entry               *TOMY*
*      CB   @CBH6A,@FAC2      Subprogram entry?                           *TOMY*
       JNE  RETURN            No, look some more
*      BL   @VPUSH            Push it back. Keep information              *TOMY*
*      JMP  ERR51             RETURN WITHOUT GOSUB error                  *TOMY*
*
* Have an ERROR entry
...
*CBH6A BYTE >6A               Subprogram call stack ID                    *TOMY*
*      EVEN                                                               *TOMY*
*************************************************************
...
*      BL   @NUMCHK           Ensure the value is a number                *TOMY*
